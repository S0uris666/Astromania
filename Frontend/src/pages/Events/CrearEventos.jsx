// src/pages/Admin/CrearEventos.jsx
import { useState, useMemo } from "react";
import { useEvents } from "../../context/events/eventsContext";

// convierte 'YYYY-MM-DDTHH:mm' (local) a ISO real (UTC)
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        url: form.url.trim(), // se envía vacío o con valor
        status: form.status,
      };

      await createOneEvent(payload); // <-- usa el contexto (actualiza store)
      alert("Evento creado ✅");
      ...
      // el context ya normaliza mensajes (si usas getHttpErrorMessage en el state)
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <button type="submit" className="btn btn-primary" disabled={submitting || loading}>
            {submitting || loading ? "Creando..." : "Crear evento"}
          </button>
        </div>
      </form>
    </main>
